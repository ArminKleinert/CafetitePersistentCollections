...
class VListTest {
    ...
    private inline fun <T> from(coll: Array<T>) = VList.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> from(coll: Iterable<T>) = VList.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> from(coll: Collection<T>) = VList.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> of(vararg xs: T) = VList.of(*xs)
    ...
    @Test
    fun reversed() {
        Assertions.assertInstanceOf(PersistentList::class.java, VList.of<Int>())
