...
class PersistentWrapperTest {
    ...
    private inline fun <T> from(coll: Array<T>) = PersistentWrapper.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> from(coll: Iterable<T>) = PersistentWrapper.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> from(coll: Collection<T>) = PersistentWrapper.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> of(vararg xs: T) = PersistentWrapper.of(*xs)
