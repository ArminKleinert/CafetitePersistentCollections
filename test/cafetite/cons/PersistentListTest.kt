...
class PersistentListTest {
    ...
    private inline fun <T> from(coll: Array<T>) = PersistentList.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> from(coll: Iterable<T>) = PersistentList.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> from(coll: Collection<T>) = PersistentList.from(coll)

    @Suppress("NOTHING_TO_INLINE")
    private inline fun <T> of(vararg xs: T) = PersistentList.of(*xs)
