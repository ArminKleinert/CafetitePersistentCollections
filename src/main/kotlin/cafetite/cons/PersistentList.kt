...
sealed class PersistentList<T> : List<T>, Iterable<T> {
    companion object {
    ...
    open fun cleared(): PersistentList<T> = nullCons()

    override fun get(index: Int): T = try {when (index) {
        0 -> car
        1 -> cdr.first()
    ...
    open fun reversed(): PersistentList<T> {
        if (isEmpty())
            return this
    ...
    protected fun commonEqualityCheck(other: Any?): Boolean {
