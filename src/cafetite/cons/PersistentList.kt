...
import cafetite.cons.ListPair
import java.util.*
import kotlin.NoSuchElementException
import kotlin.collections.reversed
import kotlin.random.Random

sealed interface PersistentList<T> : List<T> {
    companion object {
        fun <T> of(vararg elements: T): PersistentList<T> =
            if (elements.isEmpty()) nullCons() else VList.from(elements.toList())
        ...
        fun <T> from(coll: Iterable<T>): PersistentList<T> =
            when (coll) {
                ...
                else -> VList.from(coll)
            }

        fun <T> from(arr: Array<T>): PersistentList<T> = VList.from(arr)

        fun <T> randomAccess(list: Iterable<T>): PersistentList<T> = PersistentWrapper(list.toList())

        fun <T> log2Access(list: Iterable<T>): PersistentList<T> = VList.from(list)
    ...
    abstract val cdr: PersistentList<T>
    ...
    fun cleared(): PersistentList<T>

    override fun get(index: Int): T = try {
        when (index) {
            0 -> car
            1 -> cdr.first()
            in 2..<size -> drop(index).first()
    ...
    fun reversedPersistent(): PersistentList<T>
    ...
    fun <R : Any> mapIndexedNotNull(transform: (index: Int, T) -> R?): PersistentList<R> =
        ...
        LazyList.mapIndexedNotNull(transform, this)
...
fun <T> PersistentList<T>.reversed(): PersistentList<T> =
    (this as PersistentList<T>).reversedPersistent()
