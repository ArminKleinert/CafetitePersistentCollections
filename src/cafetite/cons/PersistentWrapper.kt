...
class PersistentWrapper<T> : AbstractPersistentList<T>, RandomAccess {
    ...
    companion object {
        fun <T> of(vararg elements: T): PersistentWrapper<T> =
            PersistentWrapper(elements.toList())

        fun <T> from(coll: List<T>): PersistentList<T> =
            if (coll is PersistentWrapper<T>) coll
        ...
        fun <T> from(coll: Array<T>): PersistentList<T> =
            PersistentWrapper(coll.asIterable())
    }
    ...
    override fun asIterable(): Iterable<T> = actualList.asIterable()

    override fun toList(): List<T> = actualList

    override fun asSequence(): Sequence<T> = actualList.asSequence()
