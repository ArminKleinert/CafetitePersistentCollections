...
class VList<T> private constructor(segment: Segment?, private val offset: Int) : AbstractPersistentList<T>() {
...
//
//        @Suppress("UNCHECKED_CAST")
//        fun <T> of(vararg elements: T): VList<T> =
//            if (elements.isEmpty()) EMPTY as VList<T>
//            else segmentsAndOffsetFromReversedList(elements.asIterable())

        @Suppress("UNCHECKED_CAST")
        fun <T> of(vararg elements: T): VList<T> =
            if (elements.isEmpty()) VList<T>(null, 0)
            else segmentsAndOffsetFromReversedList(elements.asIterable())

        fun <T> from(iterable: Iterable<T>): VList<T> = when (iterable) {
            is VList<T> -> iterable
            else -> segmentsAndOffsetFromReversedList(iterable)
        }

        fun <T> from(iterable: Array<T>): VList<T> =
    ...
    override fun subList(fromIndex: Int, toIndex: Int): VList<T> = from(toList().subList(fromIndex, toIndex))
    ...
    override fun <R> sameTypeFromList(list: List<R>): VList<R> = from(list)
    ...
    fun <R> mapSegments(f: (T) -> R): List<List<R>> {
        ...
        while (segment != null) {
            ...
            @Suppress("UNCHECKED_CAST")
            val elements = segment.elements as Array<T>
            for (i in offset..elements.lastIndex)
                tempList.add(f(elements[i]))
